...
export default function IncomePage() {
  ...
  const fetchData = async () => {
    ...
    setTransactions(tData.filter((t: any) => t.type === 'income'));

    const cRes = await fetch('/api/categories');
    const cData = await cRes.json();
    setCategories([...cData.defaults, ...cData.custom].filter(c => c.type === 'income'));
  };
    ...
    <div className="glass-panel" style={{ maxWidth: '800px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <h2 className="heading" style={{ margin: 0 }}>All Income</h2>
        </div>
      ...
      <div className="transaction-list">
        {transactions.length > 0 ? transactions.map(t => (
          <div key={t._id} className="transaction-item">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <span className={`t-amount income`}>
                +₹{t.amount.toFixed(2)}
              </span>
        ...
        )) : <p className="subtext">No income found for this month.</p>}
      </div>
    </div>
  );
}
